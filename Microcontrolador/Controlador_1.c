...
void main()
{
   ...
   date_time_t dt;
   //Variavel tipo estrutura que recebera a umidade/temperatura
   value humi_val,temp_val;
   //Varivel para receber o erro e o Checksum do sensor SHT11
   ...
   //Inicializa a UART com o Sensor de Temperatura
   //init_uart();
   //Reseta a conexao com o sensor
   ...
   while (true)
   {
      ...
      if (!(conta % 5)) {
         ...
         /* Todo esse bloco foi substituido por conta do novo sensor
         ...
         //Calcula a umidade pela equação de regressão estimada
         //RH = (float) (0.159*read_adc()) - 31.205;
         //T2=read_adc();

         */

         //Faz a medicao da umidade via SensiBus no SHT11
         error+=sht11_measure((unsigned char*) &humi_val.i,&checksum,HUMI);
         //Faz a medicao da temperatura via SensiBus no SHT11
         error+=sht11_measure((unsigned char*) &temp_val.i,&checksum,TEMP);
         //Se ocorrer um erro na medicao, reseta a conexao
         
         lcd_pos_xy(10,1);
         printf(escreve_lcd,"%i",error);
         if(error!=0) {
            sht11_connectionreset();
         } else {
            ...
            //Calcula a umidade e temperatura
            sth11_calc(&humi_val.f,&temp_val.f);
            //Faz a adaptaçao para as variaveis do programa
            Temp = temp_val.f;
            RH = humi_val.f;
...
//}
         ...
         printf(escreve_lcd,"%3.1fC",temp_val.f);
         lcd_pos_xy(12,2);
         //printf(escreve_lcd,"%3.1f%%",RH);
         printf(escreve_lcd,"%3.1f%%",humi_val.f);
...
*/
      ...
      /**************************************************************
      *   Aqui é feito todo o controle de irrigação                 *
      ***************************************************************/
      //tempo++;


      /*******  Esse Bloco Controla a irrigação tradicional *********/

      T_Temp  = Temp * 10;
      Temp_16 = (long) T_Temp;
      //tempo=0;

      // Irrigação de maior volume (3x ao dia)
      ...
      // Irrigações intermitentes (sprais)
...
*/
      /*******  Esse Bloco Controla a irrigação Especial *********/
/*
...
*/
      ...
      /**************************************************************
      *   Aqui é feito todo o Registro de temperatura               *
      ***************************************************************/
      ...
      if (mem_pos<8191){
         ...
         if (!(dt.hours % 4)) //Horas
         {
            if (Fregistro){
               ...
               mem_pos++;
               //Registra a hora
               escreve_eeprom(0,mem_pos,dt.hours); //Não esquecer de mudar para horas
               mem_pos++;
               //Registra a humidade
